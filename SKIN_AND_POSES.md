...
このドキュメントには、ゲームAPIで使用可能なすべてのAPI選択可能スキンとポーズが記載されています。

## 使用方法

### プレイヤーのスキンを変更する

プレイヤーのスキンを変更するには、`api.changePlayerIntoSkin(playerId, part, skinName)` を使用します：

例：
```js
api.changePlayerIntoSkin(playerId, "body", "body_0_0")
api.changePlayerIntoSkin(playerId, "head", "wizard") // ショップキーパー スキン
```

### プレイヤーのポーズを設定する

プレイヤーのポーズを変更するには、`api.setPlayerPose(playerId, pose)` を使用します:
...
以下のスキンパーツをカスタマイズできます：`head`、`body`、`legs`、`shoes`、`eyebrows`、`eyes`、`skin`

---

## NPCのスキン（9つ）

これらは通常NPCに使用される特別な全身スキンです。`head`パーツ経由で適用することができます。
...
chef
farmer //ワンブロックのフレッド
piggy_banker //ワンブロックの銀行員
portal_mage //ワンブロックのマーベリック
trader //ショップキーパー
trader_black //ショップキーパー
trader_blue //ショップキーパー
wizard //ショップキーパー
zombie //ゾンビ
```
...
- `_none` で終わるスキン名は内部バリエーションであり、API経由では使用できません。
- スキンバリエーションは `{part}_{style}_{colour}` の形式に従います（例: `body_0_0`、`head_1_2`）。
- NPCスキンはキャラクターの外見全体を適用し、個々のパーツ選択を上書きします。